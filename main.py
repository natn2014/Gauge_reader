...
from PySide6.QtWidgets import QApplication, QMainWindow, QTableWidgetItem, QDialog, QVBoxLayout, QComboBox, QPushButton, QLabel
...
class V4L2DeviceSelector:
    """Detect and manage V4L2 video devices"""
    ...
    def get_available_devices():
        ...
        if os.name == 'nt':
            return V4L2DeviceSelector._get_windows_devices()
        
        # On Linux, look for /dev/video* devices
        try:
            for i in range(10):
    ...
    def _get_windows_devices():
        ...
        for i in range(5):
            ...
            if cap.isOpened():
                devices.append({'index': i, 'name': f'Camera {i}', 'path': None})
                cap.release()
        return devices
    
    @staticmethod
    def show_device_selector():
        """Show dialog to select video device"""
        devices = V4L2DeviceSelector.get_available_devices()
        
        if not devices:
            print("No video devices found. Using default video file: gauge_video.mp4")
            return "gauge_video.mp4"
        
        # Create selection dialog
        dialog = QDialog()
        dialog.setWindowTitle("Select Video Source")
        dialog.setGeometry(100, 100, 400, 200)
        layout = QVBoxLayout()
        
        label = QLabel("Available Video Devices:")
        layout.addWidget(label)
        
        combo = QComboBox()
        selected_device = "gauge_video.mp4"
        
        # Add option to use video file
        combo.addItem("Use Video File (gauge_video.mp4)", "gauge_video.mp4")
        
        # Add available devices
        for device in devices:
            device_name = device.get('name', f"Device {device['index']}")
            combo.addItem(f"{device_name}", device['index'])
        
        layout.addWidget(combo)
        
        def on_select():
            nonlocal selected_device
            selected_device = combo.currentData()
            dialog.accept()
...
class GaugeReaderApp(QMainWindow):
    def __init__(self):
        ...
        video_source = V4L2DeviceSelector.show_device_selector()
